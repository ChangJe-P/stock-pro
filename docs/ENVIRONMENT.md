...
| 변수 | 용도 | 로컬 값(v1) |
|---|---|---|
| VIRTUAL_INITIAL_CASH_KRW | 최초 계좌 적립 가상 현금(KRW 정수, 양수) | 10000000 |
| VIRTUAL_BUY_FEE_RATE | 매수 수수료율 가정(0 이상 1 미만) | 0.00015 |
| VIRTUAL_SELL_FEE_RATE | 매도 수수료율 가정(0 이상 1 미만) | 0.00015 |
| VIRTUAL_SELL_TAX_RATE | 매도 세금율 가정(0 이상 1 미만) | 0 |
| VIRTUAL_SLIPPAGE_BPS | 슬리피지 가정(0 이상 정수 bps) | 0 |
| VIRTUAL_TRADING_POLICY_VERSION | 계좌에 저장할 정책 식별자 | v1 |

- 각 값은 backend 설정 계층에서만 읽는다. 서비스 코드·테스트 fixture에 값 자체를 직접 적지 않는다.
- 누락·형식 오류·음수 현금·0 현금·음수 비율·1 이상 수수료/세금율·음수 슬리피지는 값이나 비밀값을 노출하지 않는 안전한 설정 오류로 끝난다.
- 값은 계좌 최초 생성 시 스냅샷으로 DB에 저장된다. **이후 값을 바꿔도 기존 계좌·원장 해석에는 소급 적용되지 않는다.**
- 실제 수수료·세금 자동 동기화나 정책 수정 API는 만들지 않는다(별도 승인 작업).
